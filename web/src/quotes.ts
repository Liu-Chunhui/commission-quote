...
async function idempotencyKey(request: QuoteRequest): Promise<string> {
  const name = `${request.loanAmount}|${request.loanTermInMonths}|${request.riskBand}`;
...
function isQuote(value: unknown): value is Quote {
  return typeof value === 'object' && value !== null
    ...
    && 'commissionRate' in value && typeof value.commissionRate === 'number'
    && [0.01, 0.02, 0.03].includes(value.commissionRate)
    && 'totalCommission' in value && typeof value.totalCommission === 'number'
    && Number.isFinite(value.totalCommission);
}
