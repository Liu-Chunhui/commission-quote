...
- Place executable entry points in `cmd/`, the frontend in `web/`, and documentation in `docs/`. Keep `cmd/app/main.go` limited to startup wiring. Place the application backend implementation and its tests together in `internal/app/` using `package app`, with request handling and vendor calls in separate files.
...
- Every microservice must register `router.Get("/health", health)`. Follow the shared health-check contract in `docs/01-development-approach.md`.
...
- Aim for Go statement coverage greater than 90% in each production backend source file, including implementation files in `internal/app/`, except `main.go`; 100% coverage is not required.
